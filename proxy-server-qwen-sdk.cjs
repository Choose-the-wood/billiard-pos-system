...
// 生成阿里云签名的函数
function generateAliCloudSignature(accessKeySecret, stringToSign) {
    const crypto = require('crypto');
    return crypto.createHmac('sha1', accessKeySecret).update(stringToSign).digest('base64');
}

// 生成阿里云认证头
function generateAliCloudAuth(method, uri, headers, accessKeyId, accessKeySecret) {
    const crypto = require('crypto');
    ...
    return {
        'Authorization': `ACS3-HMAC-SHA256 Credential=${accessKeyId},SignedHeaders=content-type;host;x-acs-action;x-acs-date;x-acs-signature-nonce;x-acs-version,Signature=${signature}`,
        'X-Acs-Date': timestamp,
        'X-Acs-Signature-Nonce': nonce,
        'X-Acs-Version': '2023-03-20',
        'X-Acs-Action': 'GenerateText'
    };
}
...
app.get('/health', (req, res) => {
    res.json({
        ...
        provider: 'tencent-cloud-with-qwen-sdk'
    });
});

// 通义千问API代理 - 使用正确的API密钥认证
app.post('/api/qwen', async (req, res) => {
    try {
        ...
        console.log('🔑 使用通义千问API密钥:', apiKey.substring(0, 10) + '...');
        console.log('🔑 API密钥长度:', apiKey.length);
        console.log('🤖 调用通义千问API (使用Bearer Token认证)');
        console.log('📊 请求参数:', {
...
// 启动服务器
app.listen(PORT, () => {
    console.log(`🚀 腾讯云+通义千问代理服务器运行在端口 ${PORT}`);
    console.log(`🔍 健康检查: http://localhost:${PORT}/health`);
    console.log(`🤖 通义千问代理(SDK认证): http://localhost:${PORT}/api/qwen`);
    console.log(`🎤 腾讯云语音识别: http://localhost:${PORT}/api/speech/recognize`);
    console.log('🔧 使用腾讯云语音识别 + 阿里云SDK认证通义千问');

    // 检查环境变量
    if (!process.env.TENCENT_SECRET_ID || !process.env.TENCENT_SECRET_KEY) {
        console.log('⚠️  警告: 腾讯云密钥未配置');
    }
    if (!process.env.ALI_ACCESS_KEY_ID || !process.env.ALI_ACCESS_KEY_SECRET) {
        console.log('⚠️  警告: 阿里云AccessKey未配置');
    }
